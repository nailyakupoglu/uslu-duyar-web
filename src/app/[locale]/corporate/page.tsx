...
export function generateMetadata({ params: { locale } }: { params: { locale: string } }): Metadata {
  return buildMetadata({
    ...
    description:
      locale === "en"
        ? "Uslu Duyar corporate pages: about us, quality policy, vision & mission, and our approach to sustainability."
        : "Uslu Duyar kurumsal sayfaları: hakkımızda, kalite politikası, vizyon & misyon ve sürdürülebilirlik yaklaşımı.",
    path: "/corporate"
  });
}

export default function CorporatePage({ params: { locale } }: { params: { locale: string } }) {
  const corporateLinks = getCorporateLinksL(locale);
  const timeline = getTimelineL(locale);

  const descriptions: Record<string, string> = {
    "/corporate/hakkimizda":
      locale === "en"
        ? "Our Çukurova-rooted story of fresh produce and the values we have carried since 1991."
        : "1991'den bugüne Çukurova merkezli taze ürün hikâyemiz ve değerlerimiz.",
    "/corporate/kalite-politikasi":
      locale === "en"
        ? "Our commitment to food safety, traceability, and continuous improvement."
        : "Gıda güvenliği, izlenebilirlik ve sürekli iyileştirme taahhüdümüz.",
    "/corporate/vizyon-misyon":
      locale === "en"
        ? "Our long-term goals that grow trust from field to table."
        : "Tarladan sofraya güveni büyüten uzun vadeli hedeflerimiz.",
    "/corporate/surdurulebilirlik":
      locale === "en"
        ? "Our responsible production approach across water, energy, and the supply chain."
        : "Su, enerji ve tedarik zincirinde sorumlu üretim yaklaşımımız."
  };

  return (
    <>
      <Breadcrumb items={[{ label: locale === "en" ? "Corporate" : "Kurumsal" }]} />
      <PageHero
        eyebrow={locale === "en" ? "Corporate" : "Kurumsal"}
        title={locale === "en" ? "Three decades of expertise, corporate discipline" : "30 yıllık birikim, kurumsal disiplin"}
        description={
          locale === "en"
            ? "We present our journey from a family business to an integrated agriculture brand, together with our principles of quality and sustainability."
            : "Aile işletmesinden entegre tarım markasına uzanan yolculuğumuzu, kalite ve sürdürülebilirlik ilkelerimizle birlikte sunuyoruz."
        }
      />
