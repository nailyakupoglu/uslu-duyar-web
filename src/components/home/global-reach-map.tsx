  ...
  return (
    <section className="section-padding bg-primary-50 text-ink">
      <div className="container grid gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
        <div>
          <p className="text-sm font-bold uppercase tracking-[0.22em] text-accent-700">{locale === "en" ? "Global reach" : "Global erişim"}</p>
        ...
        <div className="rounded-lg border border-line-soft bg-white p-4 shadow-[0_24px_60px_rgba(14,14,14,0.08)]">
          <svg viewBox="0 0 100 64" role="img" aria-label={locale === "en" ? "Uslu Duyar export map" : "Uslu Duyar ihracat haritası"} className="h-auto w-full">
            ...
            {exportPoints.map((point, index) => (
              <g key={point.city}>
                ...
                <text x={point.x + 1.8} y={point.y + 0.8} fill="#161A14" fontSize="2.4">
                  {point.city}
                </text>
              </g>
            ))}
